...
public struct GSImageInfo {
    ...
    public init(image: UIImage, imageMode: ImageMode, originalURL: URL?, localURL: URL?) {
...
open class GSImageViewerController: UIViewController {
    ...
    public convenience init(image: UIImage, imageMode: UIView.ContentMode, imageHD: URL?, localURL: URL?, fromView: UIView?) {
        let imageInfo = GSImageInfo(image: image, imageMode: GSImageInfo.ImageMode(rawValue: imageMode.rawValue)!, originalURL: imageHD, localURL: localURL)
    ...
    fileprivate func setupImageHD() {
        guard let imageHD = imageInfo.originalURL, let localURL = imageInfo.localURL else { return }
        
        let request = URLRequest(url: imageHD, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 15)
        let task = session.dataTask(with: request, completionHandler: { (data, response, error) -> Void in
            guard let data = data else { return }
            
            guard let image = UIImage(data: data), let square = image.EXT_circleMasked else { return }
            do {
                try square.pngData()?.write(to: localURL)
                
                self.imageView.image = image
                self.view.layoutIfNeeded()
                
                Log("Download and updated image \(image)")
            } catch {
                Log(error)
            }
