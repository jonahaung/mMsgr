...
extension UIImageView {
    ...
    convenience init(gifImage:UIImage, manager:SwiftyGifManager = SwiftyGifManager.defaultManager, loopCount: Int = -1) {
        self.init()
        setGifImage(gifImage,manager: manager, loopCount: loopCount, animateAfterDownload: true);
    }
     ...
     */
    func setGifImage(_ gifImage: UIImage, manager: SwiftyGifManager = SwiftyGifManager.defaultManager, loopCount: Int = -1, animateAfterDownload: Bool) {
...
//        self.haveCache = false
     ...
     */
    func setGifFromURL(_ url: URL?, manager: SwiftyGifManager = SwiftyGifManager.defaultManager, loopCount: Int = -1, animateAfterDownload: Bool = false) {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, _ , _) in
            DispatchQueue.main.async {
                if let data = data {
                    self.setGifImage(UIImage.init(gifData: data), manager: manager, loopCount: loopCount, animateAfterDownload: animateAfterDownload)
