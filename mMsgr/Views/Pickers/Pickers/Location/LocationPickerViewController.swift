...
final class LocationPickerViewController: UIViewController {
    ...
    lazy var locationButton: MyButton = {
        $0.maskToBounds = true
        $0.cornerRadius = 22
        if let url = GlobalVar.currentUser?.photoURL_local, let image = UIImage(contentsOfFile: url.path) {
            $0.setImage(image, for: UIControl.State())
        } else {
             $0.setImage(UIImage(systemName: "person.circle.fill"), for: UIControl.State())
        }
       
        $0.addTarget(self, action: #selector(LocationPickerViewController.currentLocationPressed),
                         for: .touchUpInside)
        return $0
    }(MyButton(frame: CGRect(x: 0, y: 0, width: 44, height: 44)))
	...
	lazy var searchController: UISearchController = {
        ...
        $0.searchBar.textField?.textColor = UIColor.label
        $0.searchBar.textField?.setPlaceHolderTextColor(UIColor.placeholderText)
...
extension LocationPickerViewController: MKMapViewDelegate {
	...
	func selectLocationButton() -> UIButton {
		let x = UIButton(frame: CGRect(x: 0, y: 0, width: 44, height: 44))
        x.tintColor = UIColor.link
...
//        }
        x.tintColor = UIColor.systemBackground
        x.setBackgroundImage(CAShapeLayer.circle(UIColor.link, diameter: x.bounds.height/2).toImage(), for: UIControl.State())
        x.setImage(#imageLiteral(resourceName: "arrow"), for: UIControl.State())
//        button.backgroundColor = theme.secondaryTintColor
//        button.setTitleColor(theme.generalBackgroundColor, for: UIControl.State())
